...
set "TCCT_DIR=%~dp0"
set "TCCT_NOTEBOOK=%TCCT_DIR%TCCT_S78_DynamicHorizonCoreRegression.ipynb"
...
if not exist "%TCCT_NOTEBOOK%" (
  echo S78 notebook not found: %TCCT_NOTEBOOK%
  pause
  exit /b 1
)
...
start "TCCT S78 JupyterLab" /min "%JUPYTER_LAB%" "%TCCT_NOTEBOOK%" --ServerApp.root_dir="%TCCT_DIR%" --ServerApp.port=8888 --ServerApp.port_retries=10
exit /b 0
