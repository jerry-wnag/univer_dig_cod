...
set "TCCT_DIR=%~dp0"
set "TCCT_NOTEBOOK=%TCCT_DIR%TCCT_S79B_PrivateDiamondCanonicalizationRepairAudit.ipynb"
...
if not exist "%TCCT_NOTEBOOK%" (
  echo S79B notebook not found: %TCCT_NOTEBOOK%
  pause
  exit /b 1
)
...
start "TCCT S79B JupyterLab" /min "%JUPYTER_LAB%" "%TCCT_NOTEBOOK%" --ServerApp.root_dir="%TCCT_DIR%" --ServerApp.port=8888 --ServerApp.port_retries=10
exit /b 0
