...
set "TCCT_DIR=%~dp0"
set "TCCT_NOTEBOOK=%TCCT_DIR%TCCT_S86B_CombinedPolicyDevelopmentAndFreeze.ipynb"
...
set "JUPYTER_CONFIG_DIR=%TCCT_DIR%.jupyter_config_s86b"
set "JUPYTER_RUNTIME_DIR=%TCCT_DIR%.jupyter_runtime_s86b"
set "PYTHONUTF8=1"
...
if not exist "%TCCT_NOTEBOOK%" (
  echo S86B notebook not found: %TCCT_NOTEBOOK%
  pause
  exit /b 1
)
...
start "TCCT S86B JupyterLab" /min "%JUPYTER_LAB%" "%TCCT_NOTEBOOK%" --ServerApp.root_dir="%TCCT_DIR%" --ServerApp.port=8892 --ServerApp.port_retries=5
exit /b 0
