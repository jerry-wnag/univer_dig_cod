...
def main() -> int:
    receipt = ROOT / "protocol" / "replay_freeze_receipt.json"
    if receipt.exists():
        raise RuntimeError("replay already frozen")
    original_protocol_path = ARCHIVE / "frozen_protocol.json"
    protocol = json.loads(original_protocol_path.read_text(encoding="utf-8"))
    protocol.update({
        "Stage": "S140-C DIAGNOSTIC R0 adaptive source-selectivity activation envelope replay",
