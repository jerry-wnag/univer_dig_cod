...
from adaptive_geometry import apply_program, query_specs
...
def respond(task_id: str, query_number: str) -> int:
    _, public, sealed, protocol_hash = boundary()
    ...
    all_public_tasks = public["SourceTasks"] + public["TargetTasks"]
    task = next(row for row in all_public_tasks if row["TaskID"] == task_id)
    query = next((row for row in query_specs(task) if row["InputSHA256"] == request.get("InputSHA256")), None)
    if query is None or query["Input"] != request.get("Input"):
        raise RuntimeError("input outside frozen intervention grammar")
    ...
    payload = {"ProtocolSHA256": protocol_hash, "TaskID": task_id,
    ...
    print(json.dumps({"OracleResponseWritten": True, "TaskID": task_id,
                      "QueryNumber": query_number}, separators=(",", ":")), flush=True)
    return 0
