...
def build_task(
    ...
    # Neutral training geometry is intentionally reusable: every candidate in
    # the structural trio maps these dual-axis-symmetric shapes identically, so
    # reuse cannot reveal the hidden family. Freshness is enforced on the test
    # witnesses, assignments, colors, placements, and sealed outputs.
    excluded_train = OLD_TRAIN_SHAPES
...
def main() -> int:
    ...
    write_json(
        ROOT / "sealed" / "materialization_manifest.json",
        {
            ...
            "PriorNeutralTrainingGeometryReuseAllowed": True,
            "FreshnessBoundary": "FRESH_TEST_WITNESS_ASSIGNMENT_COLOR_PLACEMENT_AND_OUTPUT_NEUTRAL_TRAIN_GEOMETRY_MAY_REUSE",
