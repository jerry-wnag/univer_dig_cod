...
Grid = list[list[int]]
CONTEXT_COLORS = [6, 7, 9]


def context_color(level: int) -> int:
    return CONTEXT_COLORS[level % len(CONTEXT_COLORS)]
...
def make_input(task: dict[str, Any], spec: dict[str, Any]) -> Grid:
    ...
    for level, count in enumerate(task["ContextCounts"]):
        for value in range(count):
            grid[h - 1 - level][2 + 4 * level + value] = context_color(level)
    ...
    if kind == "CALIBRATE":
        ...
        grid[0][w - 1] = 8
    elif kind == "GLOBAL_DECISION":
        grid[0][w - 2] = 4
    elif kind == "GLOBAL_BIT0":
        grid[0][w - 2] = 8
    elif kind == "GLOBAL_BIT1":
        grid[0][w - 3] = 8
    elif kind == "NUISANCE":
        grid[0][w - 1] = 7
    elif kind == "TEST":
...
def apply_program(task: dict[str, Any], program: dict[str, Any],
    ...
    if kind == "CALIBRATE":
        level = spec["Level"]
        if level not in task.get("CorruptedCalibrationLevels", []) and program["Keys"][:level] == prefix:
            value = program["Keys"][level]
            output[5 + level][2 + 4 * level + value] = context_color(level)
        return output
    if kind == "NUISANCE":
        output[8][12 + program["Nuisance"]] = 9
        return output
    if kind in {"GLOBAL_BIT0", "GLOBAL_BIT1"}:
        bit_decision = 0 if kind == "GLOBAL_BIT0" else 1
        if program["Decision"] == bit_decision:
            output[9][len(output[0]) - 2] = 9
        return output
    transform = kind in {"TRAIN", "TEST", "GLOBAL_DECISION"} or (
...
def query_specs(task: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if task.get("GlobalDecisionProbe", False):
        specs += [{"Kind": "GLOBAL_DECISION", "Level": None, "Prefix": []}]
    if task.get("GlobalBinaryDecisionProbes", False):
        specs += [{"Kind": "GLOBAL_BIT0", "Level": None, "Prefix": []},
                  {"Kind": "GLOBAL_BIT1", "Level": None, "Prefix": []}]
