...
def main() -> int:
    ...
    adversarial_pass = role_by["ADVERSARIAL_CONCEPT_REJECTION"]["InitialConceptPreferredQueryRejectedCount"] > 0
    fallback_pass = role_by["NO_REUSABLE_CONCEPT_FALLBACK"]["RootFallbackCount"] > 0
    gate = all(row["RolePass"] for row in target_scores) and concepts_match and adversarial_pass and fallback_pass and \
