...
RelationalFeaturesS139[models_List, queryHash_String, planningDepth_Integer] := Module[
  ...
  <|"RemainingCoordinatesPositive" -> (known < coordinateCount),
    "RemainingCoordinatesZero" -> (known === coordinateCount),
    "PlanningDepthGreaterThanOne" -> (planningDepth > 1),
    "PlanningDepthEqualOne" -> (planningDepth === 1),
    "GuaranteedKnownPrefixGain" -> (Length[branchKnown] > 0 && Min[branchKnown] > known),
    "GuaranteedDecisionResolution" -> (Length[branchDecisions] > 0 && Max[branchDecisions] === 1),
    "GuaranteedDecisionProgress" -> (Length[branchDecisions] > 0 && Max[branchDecisions] < currentDecisions),
    "NonTrivialOutcomePartition" -> (Length[branchRows] > 1)|>
];

RuleMatchesS139[rule_Association, features_Association] := And @@
  (TrueQ[Lookup[features, #[[1]], False] === #[[2]]] & /@ rule["Conditions"]);

CandidateRulesS139[] := Flatten[Table[
  <|"Conditions" -> Join[{{stateKey, True}}, ({#, True} & /@ queryKeys)]|>,
  {stateKey, {"RemainingCoordinatesPositive", "RemainingCoordinatesZero",
    "PlanningDepthGreaterThanOne", "PlanningDepthEqualOne"}},
  {queryKeys, Join[Subsets[{"GuaranteedKnownPrefixGain", "GuaranteedDecisionResolution",
      "GuaranteedDecisionProgress", "NonTrivialOutcomePartition"}, {1}],
    Subsets[{"GuaranteedKnownPrefixGain", "GuaranteedDecisionResolution",
      "GuaranteedDecisionProgress", "NonTrivialOutcomePartition"}, {2}]]}], 1];

InduceConceptsS139[events_List] := Module[
  {positives = {}, negatives = {}, candidates = {}, covered, taskSupport, falseCount,
   valid, uncovered, selected = {}, ranked, chosen, body, conceptID, eventIndex = 0, positiveIndex = 0},
  Do[eventIndex++;
    Do[positiveIndex++; AppendTo[positives, <|"EventIndex" -> positiveIndex, "TaskID" -> event["TaskID"],
      "Features" -> RelationalFeaturesS139[event["Models"], queryHash, event["PlanningDepth"]]|>],
      {queryHash, event["OptimalInputSHA256"]}];
    Do[If[!MemberQ[event["OptimalInputSHA256"], queryHash], AppendTo[negatives,
      <|"EventIndex" -> eventIndex, "Features" -> RelationalFeaturesS139[event["Models"], queryHash,
        event["PlanningDepth"]]|>]],
      {queryHash, event["UnusedInputSHA256"]}], {event, events}];
  Do[covered = Select[positives, RuleMatchesS139[rule, #["Features"]] &];
    taskSupport = Length[DeleteDuplicates[Lookup[covered, "TaskID"]]];
    falseCount = Count[negatives, row_ /; RuleMatchesS139[rule, row["Features"]]];
    If[Length[covered] >= protocol["ConceptMinimumSupportEvents"] &&
      ...
      AppendTo[candidates, Join[rule, <|"CoveredEventIndices" -> Lookup[covered, "EventIndex"],
        ...
        "TrainingFalsePositiveCount" -> 0|>]]], {rule, CandidateRulesS139[]}];
  ...
  <|"LibraryType" -> "KERNEL_INDUCED_ANONYMOUS_RELATIONAL_PLANNING_CONCEPTS",
    ...
    "PrimitiveStatePredicates" -> {"RemainingCoordinatesPositive", "RemainingCoordinatesZero",
      "PlanningDepthGreaterThanOne", "PlanningDepthEqualOne"},
    "PrimitiveQueryPredicates" -> {"GuaranteedKnownPrefixGain", "GuaranteedDecisionResolution",
      "GuaranteedDecisionProgress", "NonTrivialOutcomePartition"},
...
InstantiateConceptsS139[models_List, unused_List, planningDepth_Integer] := Module[{pairs = {}, matches},
  Do[matches = Select[Sort[unused], RuleMatchesS139[concept,
      RelationalFeaturesS139[models, #, planningDepth]] &];
...
conceptSupportPass = conceptLibrary["ConceptCount"] >= 2 && And @@
...
preScorePass = sourceDepths === {3, 2, 1} && sourceQueryCount >= 3 &&
  ...
  rootConceptUseCount >= 3 && rootFallbackCount >= 1 && preferredRejectionCount >= 1 &&
