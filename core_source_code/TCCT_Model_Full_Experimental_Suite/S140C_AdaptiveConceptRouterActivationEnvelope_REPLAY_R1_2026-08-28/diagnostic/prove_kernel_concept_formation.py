...
def main() -> int:
    ...
    adversarial_rejection = adversarial_row[
        "GuidedInitialWorkCounters"]["ConceptPreferredQueryRejectedCount"] > 0
    adversarial_safe_nonactivation = adversarial_row[
        "GuidedInitialWorkCounters"]["ConceptApplicableStateCount"] == 0
    adversarial_safe = adversarial_rejection or adversarial_safe_nonactivation
    ...
    concept_support = concepts["ConceptCount"] >= 1 and all(
    ...
    passed = source_depths == [3, 2, 1] and len(events) >= 3 and concept_support and all_depth and all_exact and \
        transfer_reduction and adversarial_safe and no_reuse_fallback
    certificate = {
    ...
    print(json.dumps({key: certificate[key] for key in (
        ...
        "TransferPlanningWorkReduced", "AdversarialControlSafe",
